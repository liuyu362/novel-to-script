...
**修复操作 字段说明**：
- 修复操作 对象包含 查找文本 和 替换为 两个字段
- 查找文本：剧本中需要被替换/定位的原始文本片段（尽量精确，10字以上）
- 替换为：修改后的文本（保持原有格式，只改动需要修正的部分）
- 如果问题需要新增文本，查找文本 设为要插入位置前的文本，替换为 在原文本后追加新内容
- 如果问题需要删除文本，替换为 设为删除后的精简版本
- 如果问题无法用简单替换修复（如结构性重写），修复操作 可省略

**规则**：
- 每条 issue 必须包含 类型、严重程度、描述、建议 字段
- 关联场景 和 涉及角色 可选，但有助于定位问题时必须提供
- 修复操作 尽量提供，让用户可以直接一键修复
...
请分析剧本与原文之间、剧本内部是否存在逻辑矛盾，以 JSON 格式返回检测结果。"""
    ...
    for iss in issues:
        ...
        # 提取修复操作（如果 LLM 提供了）
        fix = iss.get("修复操作")
        if isinstance(fix, dict) and fix.get("查找文本") and fix.get("替换为"):
            item["修复操作"] = {
                "查找文本": fix["查找文本"],
                "替换为": fix["替换为"],
            }
